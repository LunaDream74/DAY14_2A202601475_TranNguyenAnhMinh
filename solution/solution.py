...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "object mapping each rubric criterion to a score from 0.0 to 1.0.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
        ...
        try:
            ...
            return {"scores": fallback, "reasoning": str(raw_response)}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(response_averages) >= 2:
            pairs = list(zip(response_averages[::2], response_averages[1::2]))
            positional_bias = bool(pairs) and all(
                first > second for first, second in pairs
            )

        average_score = sum(all_scores) / len(all_scores) if all_scores else 0.5
        return {
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def metric_average(results: list, metric: str) -> float:
            if not results:
                return 0.0
            return sum(float(getattr(result, metric)) for result in results) / len(
                results
            )
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            rows.append(
                f"| {markdown_cell(failure_id)} | "
                f"{markdown_cell(failure_type)} | "
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_by_type = {
            ...
            "irrelevant": (
                "Improve intent detection and add prompt examples for direct answers"
            ),
            "incomplete": (
                "Improve retrieval coverage and require all expected conditions in answers"
            ),
            "off_topic": (
                "Add query routing and scope checks before answer generation"
            ),
            "refusal": (
                "Calibrate guardrails with valid in-scope examples to reduce false refusals"
            ),
            "unknown": (
                "Review uncategorized failures and extend the failure taxonomy"
            ),
        }
        ...
        for failure_type, _ in prioritized_categories:
            suggestion = suggestion_by_type.get(
                failure_type.lower(),
                f"Review {failure_type} failures and add targeted regression tests",
            )
        ...
        general_suggestions = [
            ...
            "Review low-scoring retrieval traces before changing the generation prompt",
            "Track metric changes against a fixed baseline after each pipeline update",
        ]
